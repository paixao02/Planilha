...
function render(){
  ...
  document.body.innerHTML=`<div class="shell"><aside class="sidebar"><div class="brand"><span>💼</span><b>FinanceApp</b></div><button class="quick" id="quickAdd">+</button>${navHtml()}<div class="profile"><div class="avatar">${(state.user.email||'U')[0].toUpperCase()}</div><div><b>${state.user.email?.split('@')[0]||'Usuário'}</b><small>${state.user.email||''}</small></div></div></aside><main><header><div><h1>${page.title}</h1><p>${page.sub}</p></div><div class="header-actions"><input type="month" id="month" value="${state.month}"><button id="logout" class="ghost">Sair</button></div></header>${page.html()}</main><nav class="bottom">${bottomNavHtml()}</nav></div><div id="modal"></div><div id="toast"></div>`;
  bindCommon(); page.bind?.();
}
const nav = [
 ['dashboard','Visão Geral','⌂'], ['transactions','Transações','↕'], ['reports','Relatórios','▥'], ['budgets','Orçamentos','◎'], ['recurrent','Recorrências','↻'], ['goals','Metas','☆'], ['accounts','Contas','▣'], ['cards','Cartões','▤'], ['categories','Categorias','◇'], ['settings','Config','⚙']
];
...
function categoriesPage(){return {title:'Categorias',sub:'Categorias automáticas com ícones',html(){return `<div class="panel categories">${Object.entries(iconMap).map(([c,i])=>`<div><span>${i}</span><b>${c}</b><small>${money(group(txMonth()).find(x=>x[0]===c)?.[1]||0)}</small></div>`).join('')}</div>`}}}
...
const pages={
  dashboard:dashboardPage(), transactions:transactionsPage(), reports:reportsPage(),

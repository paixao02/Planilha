...
const rules = [
 ['salário|salario|pagamento|pix recebido|recebi','Salário'],
 ['mercado|supermercado|padaria|ifood|lanche|restaurante','Alimentação'],
 ['uber|99|ônibus|onibus|combustível|combustivel|gasolina','Transporte'],
 ['farmácia|farmacia|remédio|remedio|médico|medico','Saúde'],
 ['netflix|spotify|prime|assinatura|internet|celular','Assinaturas'],
 ['shopee|mercado livre|amazon|magalu|compra','Compras'],
 ['aluguel|energia|água|agua|condomínio|condominio','Casa']
];
...
function render(){
 if(!user){
  ...
  <div class="login">
   <div class="panel">
    <h1>FinanceApp Pro</h1>
    <p>Entre com e-mail e senha para sincronizar no PC e celular.</p>

    <input id="email" class="search" type="email" placeholder="Seu e-mail">
    <input id="password" class="search" type="password" placeholder="Sua senha">

    <button class="btn" id="login">Entrar</button>
    <button class="btn ghost" id="register">Criar conta</button>
   </div>
  </div>`;
...
function config(){return `<div class="panel"><h2>Configurações</h2><p>✅ Firebase conectado</p><p>✅ Login e-mail/senha ativo</p><p>✅ Dados salvos em: <b>users/${user.uid}/transactions</b></p><p>Para instalar no iPhone: Safari → Compartilhar → Adicionar à Tela de Início.</p></div>`}
...
function bind(){
 ...
 if(f){
  document.getElementById('desc').oninput=e=>{
   ...
   if(!c.value)c.value=autoCategory(e.target.value);
  };
  ...
  f.onsubmit=async e=>{
   ...
   await addDoc(collection(db,'users',user.uid,'transactions'),{
    ...
    type:d.type,
    category:d.category||autoCategory(d.description),
    date:d.date,
 ...
 if(csv)csv.onclick=()=>download('transacoes.csv',['descricao,valor,tipo,categoria,data',...filtered().map(t=>`"${t.description}",${t.amount},${t.type},"${t.category}",${t.date}`)].join('\n'));

 const backup=document.getElementById('backup');
 if(backup)backup.onclick=()=>download('backup-financeapp.json',JSON.stringify(transactions,null,2));
}

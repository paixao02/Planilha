...
function importPage(){return {title:'Importar CSV',sub:'Importe seus arquivos antigos sem alterar os valores',html(){return `<div class="panel import-panel"><h2>Importar transações</h2><p class="muted">Reconhece CSV com ponto e vírgula, valor brasileiro com vírgula, datas dd/mm/aaaa e colunas como Data;Tipo;Descrição;Categoria;Método de Pagamento;Valor;Observações.</p><input type="file" id="csvFile" accept=".csv,.txt,.json"><div class="form-grid"><label>Formato<select id="importFormat"><option value="auto">Detectar automaticamente</option><option value="mobills">Mobills</option><option value="granazul">GranAzul</option><option value="generic">CSV genérico</option></select></label><label>Separador<select id="sep"><option value="auto">Detectar</option><option value=",">Vírgula (,)</option><option value=";">Ponto e vírgula (;)</option></select></label></div><label class="check"><input type="checkbox" id="replaceMonth" checked> Substituir transações dos meses importados antes de importar</label><div class="import-actions"><button id="previewImport">Pré-visualizar totais</button><button class="primary" id="runImport">Importar agora</button><button id="downloadModel">Baixar modelo CSV</button><button class="danger" id="clearTx">Limpar transações</button><button class="danger" id="clearAll">Limpar tudo</button></div><div id="importResult" class="empty">Nenhum arquivo selecionado.</div><h3>Modelo aceito</h3><pre class="csv-example">Data;Tipo;Descrição;Categoria;Método de Pagamento;Valor;Observações\n08/07/2026;Receita;brxbet;aposta lucro;Outro;4,26;\n07/07/2026;Despesa;mercado;comida;Pix;50,00;</pre></div>`},bind(){ $('#previewImport').onclick=previewCSVImport; $('#runImport').onclick=handleCSVImport; $('#downloadModel').onclick=()=>download('modelo-importacao.csv','Data;Tipo;Descrição;Categoria;Método de Pagamento;Valor;Observações\n08/07/2026;Receita;brxbet;aposta lucro;Outro;4,26;\n07/07/2026;Despesa;mercado;comida;Pix;50,00;'); $('#clearTx').onclick=clearTransactionsOnly; $('#clearAll').onclick=clearUserData}}}
function parseCSVLine(line,sep){line=String(line||'').replace(/^\uFEFF/,'');const out=[];let cur='',q=false;for(let i=0;i<line.length;i++){const ch=line[i];if(ch==='"'){if(q&&line[i+1]==='"'){cur+='"';i++;continue}q=!q;continue}if(ch===sep&&!q){out.push(cur.trim());cur='';continue}cur+=ch}out.push(cur.trim());return out}
function detectSep(text){const first=(text.split(/\r?\n/).find(Boolean)||'').replace(/^\uFEFF/,'');return (first.split(';').length>=first.split(',').length)?';':','}
function cleanKey(h){return String(h||'').replace(/^\uFEFF/,'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9]+/g,' ').trim()}
function val(row,...keys){for(const k of keys){const nk=cleanKey(k); if(row[nk]!==undefined && row[nk]!==null && String(row[nk]).trim()!=='') return row[nk]} return ''}
...
function normalizeMoney(v){v=String(v??'0').replace(/R\$/gi,'').replace(/\u00a0/g,'').replace(/\s/g,'').trim(); if(!v)return 0; if(/^\(.*\)$/.test(v))v=v.slice(1,-1); if(v.startsWith('-'))v=v.slice(1); v=v.replace(/[^0-9,.-]/g,''); if(v.includes(',')&&v.includes('.')) v=v.replace(/\./g,'').replace(',','.'); else if(v.includes(',')) v=v.replace(',','.'); const n=Number(v); return Math.abs(isNaN(n)?0:n)}
function detectType(row){const t=String(val(row,'tipo','type','natureza','movimento')).toLowerCase(); const amountRaw=String(val(row,'valor','amount','valor r$','valor (r$)','total')); if(t.includes('rece')||t.includes('entrada')||t.includes('income'))return 'income'; if(t.includes('desp')||t.includes('saida')||t.includes('saída')||t.includes('expense'))return 'expense'; if(amountRaw.trim().startsWith('-'))return 'expense'; return 'expense'}
function parseImportFile(text,sepOpt='auto'){const sep=sepOpt==='auto'?detectSep(text):sepOpt; const lines=text.split(/\r?\n/).filter(l=>l.trim()); if(lines.length<2)return []; const headers=parseCSVLine(lines[0],sep).map(cleanKey); const rows=lines.slice(1).map(line=>{const vals=parseCSVLine(line,sep); const obj={}; headers.forEach((h,i)=>obj[h]=vals[i]||''); return obj}); return rows.map(r=>{const desc=val(r,'descricao','descrição','description','nome','titulo','título','historico','histórico','observacao','observações','observacoes')||'Importado'; const type=detectType(r); const amount=normalizeMoney(val(r,'valor','amount','valor r$','valor (r$)','total')); return {date:normalizeDate(val(r,'data','date','vencimento','data da transacao','data da transação')),description:desc,type,amount,category:val(r,'categoria','category')||autoCategory(desc,type),payment:val(r,'metodo de pagamento','método de pagamento','pagamento','payment','conta','carteira')||'Outro',tags:val(r,'tags','tag')||'',notes:val(r,'observacoes','observações','observacao','observação')||'Importado via CSV',createdAt:serverTimestamp(),updatedAt:serverTimestamp()}}).filter(x=>x.amount>0)}
function importSummary(rows){const inc=rows.filter(r=>r.type==='income').reduce((s,r)=>s+r.amount,0); const exp=rows.filter(r=>r.type==='expense').reduce((s,r)=>s+r.amount,0); const months=[...new Set(rows.map(r=>monthKey(r.date)))].sort(); return {inc,exp,count:rows.length,months}}
async function getRowsFromSelectedFile(){const f=$('#csvFile').files[0]; if(!f){showToast('Escolha um arquivo CSV.');return []} const text=await f.text(); if(f.name.toLowerCase().endsWith('.json')){try{const data=JSON.parse(text);return (data.transactions||data.transacoes||data||[]).map(r=>({...r,amount:Number(r.amount||r.valor||0)})).filter(r=>r.amount>0)}catch(e){showToast('JSON inválido.');return []}} return parseImportFile(text,$('#sep').value)}
async function previewCSVImport(){const rows=await getRowsFromSelectedFile(); if(!rows.length)return; const s=importSummary(rows); $('#importResult').innerHTML=`<div class="success"><b>Prévia encontrada:</b><br>${s.count} transações<br>Receitas: <b>${money(s.inc)}</b><br>Despesas: <b>${money(s.exp)}</b><br>Meses: ${s.months.join(', ')}</div>`}
async function handleCSVImport(){const rows=await getRowsFromSelectedFile(); if(!rows.length)return; await importRows(rows)}
async function importRows(rows){if(!rows.length)return showToast('Nenhuma transação válida encontrada.'); const s=importSummary(rows); $('#importResult').innerHTML=`Importando ${rows.length} transações...`; if($('#replaceMonth')?.checked){const months=new Set(s.months); const old=state.transactions.filter(t=>months.has(monthKey(t.date))); for(const t of old){await deleteDoc(doc(db,'users',state.user.uid,'transactions',t.id))}} for(const r of rows){await addDoc(uidPath('transactions'),r)} $('#importResult').innerHTML=`✅ ${rows.length} transações importadas.<br>Receitas: ${money(s.inc)} | Despesas: ${money(s.exp)}.`; state.page='transactions'; setTimeout(render,800)}


async function clearUserData(){
  const ok = confirm('Tem certeza que deseja apagar TODOS os dados da sua conta? Isso remove transações, orçamentos, metas, contas, cartões e recorrências.');
  if(!ok) return;
  const collections = ['transactions','budgets','goals','accounts','cards','recurrent'];
  try{
    showToast('Limpando dados... aguarde.');
    for(const name of collections){
      const items = state[name] || [];
      for(const item of items){
        if(item.id) await deleteDoc(doc(db,'users',state.user.uid,name,item.id));
      }
    }
    localStorage.removeItem('fa_month');
    showToast('Dados limpos. Agora você pode importar seus CSVs novamente.');
    setTimeout(()=>{ state.page='import'; render(); }, 800);
  }catch(e){
    showToast(e.message || 'Erro ao limpar dados.');
  }
}
async function clearTransactionsOnly(){
  const ok = confirm('Apagar somente TODAS as transações? Orçamentos, metas, contas e cartões serão mantidos.');
  if(!ok) return;
  try{
    showToast('Apagando transações...');
    for(const item of state.transactions){
      if(item.id) await deleteDoc(doc(db,'users',state.user.uid,'transactions',item.id));
    }
    showToast('Transações apagadas. Importe os CSVs novamente.');
    setTimeout(()=>{ state.page='import'; render(); }, 800);
  }catch(e){ showToast(e.message || 'Erro ao apagar transações.'); }
}

function settingsPage(){return {title:'Configurações',sub:'Firebase, backup e instalação',html(){return `<div class="panel"><h2>Conta</h2><p>${state.user.email}</p><p>Dados salvos em <b>users/${state.user.uid}</b></p><button id="backup2">Baixar backup</button><button class="danger" id="clearTx2">Limpar transações</button><button class="danger" id="clearAll2">Limpar tudo</button><button id="logout2">Sair</button><h2>iPhone</h2><p>Safari → Compartilhar → Adicionar à Tela de Início.</p></div>`},bind(){ $('#backup2').onclick=()=>download('backup-financeapp.json',JSON.stringify(state,null,2)); $('#clearTx2').onclick=clearTransactionsOnly; $('#clearAll2').onclick=clearUserData; $('#logout2').onclick=()=>signOut(auth)}}}
const pages={
